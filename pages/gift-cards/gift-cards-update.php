...
if(!empty($_POST["update"])) {
	$pdo_statement=$pdo_conn->prepare("update tbl_giftcard set giftcard_number='" . $_POST[ 'giftcard_number' ] . "', value='" . $_POST[ 'value' ]. "', description='" . $_POST[ 'description' ]. "', customer_name='" . $_POST[ 'customer_name' ]. "' where giftcard_number=" . $_GET["giftcard_number"]);
  $result = $pdo_statement->execute();
  if($result) {
		echo "<script>window.location.href='index.php?page=gift-cards'</script>";
	}
}
$pdo_statement = $pdo_conn->prepare("SELECT * FROM tbl_giftcard where giftcard_number=" . $_GET["giftcard_number"]);
...
<form method="POST" class="w-full max-w-3/4 border-collapse border border-slate-50">
      <div class="pl-10 mb-7 bg-gray-50 flex flex-wrap py-3">
        ...
        <h3 class="text-lg leading-6 font-medium text-gray-500">Giftcard Information</h3>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
            <a href="index.php?page=item-kits" class="text-2xl font-semibold text-gray-900">Update Giftcards</a>
        </div>
        <div class="py-6">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 border-gray-200 rounded-lg h-96">
                <form action="" method="POST">
                    <div class="relative z-0 mb-6 w-full group">
                        <input type="number" name="giftcard_number" value="<?php echo $result[0]['giftcard_number']; ?>"
                    ...
                    <div class="relative z-0 mb-6 w-full group">
                        <input type="text" name="value" value="<?php echo $result[0]['value']; ?>"
                            class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-black dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                            placeholder=" " required>
                        <label for="value"
                    ...
                    <div class="relative z-0 mb-6 w-full group">
                        <input type="text" name="customer_name" value="<?php echo $result[0]['customer_name']; ?>"
                            class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-black dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                            placeholder=" " required>
