...
if(!empty($_POST["update"])) {
	$pdo_statement=$pdo_conn->prepare("update tbl_employee set person_id='" . $_POST[ 'person_id' ] . "', name='" . $_POST[ 'name' ]. "', email='" . $_POST[ 'email' ]. "', phone='" . $_POST[ 'phone' ]. "', address='" . $_POST[ 'address' ] . "' where person_id=" . $_GET["person_id"]);
  $result = $pdo_statement->execute();
  if($result) {
		echo "<script>window.location.href='index.php?page=employees'</script>";
	}
}
$pdo_statement = $pdo_conn->prepare("SELECT * FROM tbl_employee where person_id=" . $_GET["person_id"]);
...
<form method="POST" class="w-full max-w-3/4 border-collapse border border-slate-50">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
            <a href="index.php?page=item-kits" class="text-2xl font-semibold text-gray-900">Update Employees</a>
        </div>
        <div class="py-6">
