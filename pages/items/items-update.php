...
if(!empty($_POST["update"])) {
	$pdo_statement=$pdo_conn->prepare("update tbl_items set item_id='" . $_POST[ 'item_id' ] . "', item_name='" . $_POST[ 'item_name' ] . "', cost_price='" . $_POST[ 'cost_price' ]. "', selling_price='" . $_POST[ 'selling_price' ]. "', quantity='" . $_POST[ 'quantity' ]. "' where item_id=" . $_GET["item_id"]);
  $result = $pdo_statement->execute();
  if($result) {
		echo "<script>window.location.href='index.php?page=items'</script>";
	}
}
$pdo_statement = $pdo_conn->prepare("SELECT * FROM tbl_items where item_id=" . $_GET["item_id"]);
...
<form method="POST" class="w-full max-w-3/4 border-collapse border border-slate-50">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
            <a href="index.php?page=item-kits" class="text-2xl font-semibold text-gray-900">Edit items-</a>
        </div>
        <div class="py-6">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 border-gray-200 rounded-lg h-96">
                <form action="" method="POST">
                    <div class="relative z-0 mb-6 w-full group">
                        <input type="text" name="item_id" value="<?php echo $result[0]['item_id']; ?>"
                    ...
                    <div class="relative z-0 mb-6 w-full group">
                        <input type="text" name="item_name" value="<?php echo $result[0]['item_name']; ?>"
                            class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-black dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                    ...
                    <div class="relative z-0 mb-6 w-full group">
                        <input type="text" name="quantity" value="<?php echo $result[0]['quantity']; ?>"
                            class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-black dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                            placeholder=" " required>
                        <label for="quantity"
