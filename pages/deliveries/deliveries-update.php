...
if(!empty($_POST["update"])) {
	$pdo_statement=$pdo_conn->prepare("update tbl_deliveries set status='" . $_POST[ 'status' ] . "', fullname='" . $_POST[ 'fullname' ]. "', address='" . $_POST[ 'address' ]. "', delivery_employee='" . $_POST[ 'delivery_employee' ]. "', category='" . $_POST[ 'category' ]. "' where sale_id=" . $_GET["sale_id"]);
  $result = $pdo_statement->execute();
  if($result) {
		echo "<script>window.location.href='index.php?page=deliveries'</script>";
	}
}
$pdo_statement = $pdo_conn->prepare("SELECT * FROM tbl_deliveries where sale_id=" . $_GET["sale_id"]);
...
<form method="POST" class="w-full max-w-3/4 border-collapse border border-slate-50">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 md:px-8">
            <a href="index.php?page=items" class="text-2xl font-semibold text-gray-900">Update Deliveries</a>
        </div>
        <div class="py-6">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 border-gray-200 rounded-lg h-96">
                <form action="" method="POST">
                    <div class="relative z-0 mb-6 w-full group">
                        <input type="text" name="sale_id" value="<?php echo $result[0]['sale_id']; ?>"
                    ...
                    <div class="relative z-0 mb-6 w-full group">
                        <input type="text" name="status" value="<?php echo $result[0]['status']; ?>"
                            class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-black dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                            placeholder=" " required>
                        <label for="status"
                    ...
                    <div class="relative z-0 mb-6 w-full group">
                        <input type="text" name="delivery_employee" value="<?php echo $result[0]['delivery_employee']; ?>"
                            class="block py-2.5 px-0 w-full text-sm text-gray-900 bg-transparent border-0 border-b-2 border-gray-300 appearance-none dark:text-black dark:border-gray-600 dark:focus:border-blue-500 focus:outline-none focus:ring-0 focus:border-blue-600 peer"
                            placeholder=" " required>
